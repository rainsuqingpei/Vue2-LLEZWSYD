        ...
        <div class="ez-container-left">
          <div class="ez-left-title flex-col-row-between">
            <div class="ex-title-tab">
              <span :class="{ on: curTab == item.value, big: item.value == '1,2,3' }" v-for="item in tabList"
                :key="item.value" @click="changeTab(item)"><i></i>{{ item.name }}</span>
            </div>
          </div>
          <div class="ez-left-list flex-wrap">
            <template v-if="buLists.length > 0">
              <!-- jump字段控制直接跳转还是进入子集 -->
              <div :class="{ 'ez-left-col-item': item.jump == 'Y', 'sr-list_col': item.jump == 'N' }"
                v-for="item in buLists" :key="item.value" @click="item.jump == 'Y' ? getChild(item) : goModule(item)">
                <div :class="{ 'ez-left-item': item.jump == 'Y', 'sr-item flex-col-align': item.jump == 'N' }">
...
export default {
  ...
  created() {
    // 逻辑-默认areaCode=2112 在省内通办 点击下属菜单时候 eProvince.vue L236设置存储sessionStorage areaCode
    sessionStorage.setItem("env", "tlHomes.html");
    sessionStorage.setItem("areaCode", "2112");
    this.areaCode = "2112";
  ...
  methods: {
    ...
    GetCityBusinessTree(businessType) {
      let params = {
      ...
      POSTbyJSON("/bmModuleInfo/GetCityBusinessTree", params).then((res) => {
        if (res.success) {
          ...
          if (businessType == 0) {
            this.buLists = this.buLists.slice(0, 9)
          }
        } else {
        }
      });

...
import { get, Delete, post, POSTbyJSON } from "@/assets/js/http";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getFaceProving: function () {
      ...
      var params = {
        ...
        macAddress: sessionStorage.getItem("mac"), //mac地址
        fwzCode: sessionStorage.getItem("fwzCode"), //服务站code
        newaddress: that.userInfo.newaddress, //新地址
        moduleCode: sessionStorage.getItem('moduleCode'), //模块编码
      };

      POSTbyJSON("/visitorInfo/api/imageCheck", params).then((res) => {
        // var vm = this;
        let moduleUrl = sessionStorage.getItem("moduleUrl");
